...
SVWS-EduGate trennt zwei unterschiedliche Zugänge:

- Eine **Admin-Oberfläche** für die Verwaltung der Mandantenhierarchie durch
  Dienstleister-Admins. Diese Control-Plane-Oberfläche ist aktuell nutzbar und wird in
  [`administration/README.md`](../administration/README.md) beschrieben.
- Ein **Gateway** für den gesicherten, mandantengetrennten Zugriff auf Schuldaten über die
  SVWS-Server-API. Dieses Gateway ist aktuell noch ein Skeleton mit OIDC-, Health- und
  `/ping`-Prüfung; echte Proxy-Logik auf die SVWS-External-API folgt später.

Anwender im Sinne dieser Seite sind perspektivisch in erster Linie Nutzer des Gateways: Systeme
oder Personen, die über eine gesicherte Schnittstelle Schuldaten einer oder mehrerer Schulen
abrufen, ohne selbst Verwaltungsrechte für Schulträger, Schulen oder SVWS-Instanzen zu benötigen.
...
| Zugriff | Verwaltung von Schulträgern, Schulen, SVWS-Instanzen, Schuldatenbanken/Schemata | Zielbild: lesender Zugriff auf Schuldaten einer freigegebenen Schule |
...
Im Zielbild verlassen die Zugangsdaten der SVWS-Server zu keinem Zeitpunkt das Backend. Clients
...
- **Gateway-Proxying** – die echte Weiterleitung auf die SVWS-External-API mit Scope-Prüfung,
  Mandantenauflösung, Credential-Injection und Gateway-Audit ist noch nicht umgesetzt.
